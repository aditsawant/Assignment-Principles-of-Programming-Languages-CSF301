...
typeExp* populateTex(parseTree* dec, parseTree* tree){
	printf("Inside popTex\n");
	symbol sym = *(tree->sym);
	typeExp* tex = (typeExp*) malloc(sizeof(typeExp));
	if(sym.is_terminal){
		dec->dtype = 0;
		printf("Inside prim dtype section\n");
		...
		printf("Exiting prim dtype section\n");
	}
	else{
		//sym is a non-terminal, then what?
		if(strcmp(sym.nt, "REC_ARRAY") == 0){
			printf("Inside rec_array section\n");
			dec->dtype = 1;
			parseTree* temp = tree;
			...
			printf("Exiting rec_array section\n");
			//test for rect array
			/*
			printf("%d\n", tex->ra.dimensions);
			...
			*/
			...
			printf("Inside jagged_array section\n");
			dec->dtype = 2;
			tree = tree->child;
			...
			while(counter--){
				...
				oldtemp = temp;
				printf("Exiting jagged_array section\n");
			}
			printf("Entering valJA\n");
...
void traverseParseTreeA(parseTree* tree){
	printf("......Inside tPtA now\n");
	if(tree == NULL || strcmp(tree->sym->nt,"ASSIGNMENT_STATEMENTS") == 0) {
		printf("Exiting tPtA\n");
		return; 
	}
	if(strcmp(tree->sym->nt,"declare") == 0){
        ...
        while(strcmp(ptr->tok.lexeme,":")!=0){
            printf("Inside the while loop for Colon\n");
			ptr = ptr->sibling;
            tentativeTableSize++;
            //printf("%d\n",tentativeTableSize);
        }
		printf("Outside the while loop for Colon\n");
        //to populate tex
		parseTree* temp = tree;
		while(temp->sibling != NULL) temp = temp->sibling;
		temp = temp->child->child;
		printf("Entering popTex\n");
		parseTree* temptree = tree;
		tree->tex = populateTex(temptree,temp);
		printf("Exited popTex\n");
...
typeElement* traverseParseTree(typeElement* table, parseTree* tree){
	parseTree* temptree = tree;
	calculateDepth(temptree,0);
	printf("Really exited calculateDepth\n");
	temptree = tree;
	traverseParseTreeA(temptree);
	printf("Exited tPtA\n");
    //printf("%d\n",tentativeTableSize);
	temptree = tree;
...
*/  
	int option;
	llnode* Ga = readGrammar("grammar.txt", Ga), *G;
	tokenStream* heada = tokeniseSourcecode("sourcecode3.txt", heada), *head;
    ...
    while(1){
        ...
        if(option == 0 ){
            ...
            table = traverseParseTree(table,tree);
			table = NULL;
			printf("On the line before printParseTree in opt 3\n");
            printParseTree(tree);
			printf("\n\n Done3 \n\n");
        }
        else if(option == 4){
